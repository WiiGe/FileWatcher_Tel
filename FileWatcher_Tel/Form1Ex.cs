...
namespace FileWatcher
{
	public partial class Form1Ex : Form
	{
        ...
        public static void InvokeEMailSend(MailConfiguration argMConfig, TimeSpan argTS) 
        {
            ...
            foreach (DictionaryEntry itemp in MailConfiguration._Recipient)
            {
                MailAddress tempAddress = new MailAddress((string)itemp.Value, (string)itemp.Key, System.Text.Encoding.UTF8);
                mailBoxCollection.Add(tempAddress);
                Console.WriteLine("Key = {0}, Value = {1}", itemp.Key, itemp.Value);
            }

            //Build Message Object List
            foreach (MailAddress tempAdd in mailBoxCollection)
            {
                ...
                tempMeaage.BodyEncoding = System.Text.Encoding.UTF8;
                tempMeaage.IsBodyHtml = true;
                smtpClient.DeliveryMethod = SmtpDeliveryMethod.Network;
                smtpClient.Send(tempMeaage);
            }
        }
